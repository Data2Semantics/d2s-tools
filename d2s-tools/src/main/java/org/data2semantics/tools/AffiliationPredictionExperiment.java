			...
			for (DataSetParameters params : dataSetsParams) {
				...
				for (int i = 0; i < 3; i++) {
					if (experimenter.hasSpace()) {		
						...
						exp = new ClassificationExperiment(dataset, new IntersectionPartialSubTreeKernel(dataset.getGraphs(), dataset.getRootVertices(), i, 0.01), seeds, cs, new FileOutputStream(file));
						experimenter.addExperiment(exp);
						results.add(exp.getResults());
					}
				}
				...
				for (int i = 1; i < 3; i++) {
					if (experimenter.hasSpace()) {		
						...
						exp = new ClassificationExperiment(dataset, new IntersectionGraphPathKernel(dataset.getGraphs(), i, 1), seeds, cs, new FileOutputStream(file));
						experimenter.addExperiment(exp);
						results.add(exp.getResults());
					}
				}				
				...
				for (int i = 1; i < 3; i++) {
					if (experimenter.hasSpace()) {		
						...
						exp = new ClassificationExperiment(dataset, new IntersectionGraphWalkKernel(dataset.getGraphs(), i, 1), seeds, cs, new FileOutputStream(file));
						experimenter.addExperiment(exp);
						results.add(exp.getResults());
					}
				}
